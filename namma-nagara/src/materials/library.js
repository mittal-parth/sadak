...
import { TARPAULIN, PLASTER_PAINTED, MOSS_CONCRETE } from './glsl/surfaces-india.js';
...
export const LIBRARY = {
  ...
  // ============================================================== india ====
  // See glsl/surfaces-india.js for why these three. Note what is NOT here:
  // the vertical monsoon staining below every ledge is the SHADER's job
  // (`weather[1]` + `grimeColor`), not the tile's — so every Indian entry
  // below runs a high streak term and a green-black grime colour.

  /**
   * Blue woven polyethylene tarp. Stall roofs, load covers, leak patches.
   * A 0.5 m tile: the weave is 3 mm tape, so this is the resolution where the
   * over-under is ~6 px and still resolves before it aliases.
   */
  tarpaulin: {
    glsl: TARPAULIN,
    surface: 'fabric',
    bake: { size: 1024, worldSize: 0.5, relief: 0.012, seed: 131, tintA: 0x2f6ea8 },
    mat: {
      scale: 0.5,
      parallax: 0.004,
      detile: 0.5,
      detail: [12, 0.5, 0.3, 20],
      macro: [0.10, 0.34, 0.16, 0.4],
      // Big folds at 1.5 m so a stall roof is not one flat value across its span.
      macroBig: [1.55, 0.135, 0.03, 0],
      weather: [0.2, 0.35, 0.25, 0.4],
      dustColor: 0x8a8172,
      grimeColor: 0x1a1d16,
      // Plastic keeps a real specular lobe — this is what separates it from cloth.
      roughness: [0.9, -0.02, 0.3],
    },
  },
  /**
   * Yellow tarp — the auto-rickshaw canopy.
   *
   * This is a THIRD BAKE rather than a tint on the blue one, and it has to be.
   * `opts.tint` is a linear multiply on the baked albedo, so tinting the blue
   * tarp yellow multiplies blue by yellow and gives you green. Any tarp colour
   * that is not a darkening of the base has to be its own bake.
   */
  tarpaulin_yellow: {
    glsl: TARPAULIN,
    surface: 'fabric',
    bake: { size: 1024, worldSize: 0.5, relief: 0.012, seed: 139, tintA: 0xe8b830 },
    mat: {
      scale: 0.5,
      parallax: 0.004,
      detile: 0.5,
      detail: [12, 0.5, 0.3, 20],
      macro: [0.1, 0.34, 0.16, 0.4],
      macroBig: [1.55, 0.135, 0.03, 0],
      weather: [0.2, 0.35, 0.25, 0.4],
      dustColor: 0x8a8172,
      grimeColor: 0x1a1d16,
  ...
  /** Orange tarp — the other colour you actually see, on carts and lorries. */
  tarpaulin_orange: {
    glsl: TARPAULIN,
    surface: 'fabric',
    bake: { size: 1024, worldSize: 0.5, relief: 0.012, seed: 137, tintA: 0xc25a1e },
    mat: {
      scale: 0.5,
      parallax: 0.004,
      detile: 0.5,
  ...
  /**
   * Distemper/emulsion over cement render, chalked and peeling.
   * `tintA` is the wall colour — the palette makes one entry per colour on the
   * street, all sharing this glsl and differing only in tint and seed.
   */
  plaster_painted: {
    glsl: PLASTER_PAINTED,
    surface: 'plaster',
    bake: { size: 1024, worldSize: 2.2, relief: 0.055, seed: 149, tintA: 0xd8cfbc },
    mat: {
      scale: 2.2,
      parallax: 0.013,
      detile: 0.8,
      detail: [8, 0.85, 0.5, 24],
      macro: [0.09, 0.6, 0.24, 0.5],
      macroBig: [2.1, 0.125, 0.028, 0],
      patch: [0.3, 2.2, 0.15, -0.09],
      // Streak term is nearly double the desert plaster's. Bengaluru gets
      // ~970 mm of rain a year and every ledge runs a black tail below it.
      weather: [0.35, 0.85, 0.62, 0.55],
      wearColor: 0x9c9990,
      dustColor: 0x8a8172,
      grimeColor: 0x14180f,
      roughness: [0.97, -0.02, 0.22],
    },
  },

  /**
   * Damp, algae-colonised concrete. The baseline hard surface of the level —
   * plinths, compound walls, storm-drain slabs, unfinished RCC.
   */
  moss_concrete: {
    glsl: MOSS_CONCRETE,
    surface: 'concrete',
    bake: { size: 1024, worldSize: 2.5, relief: 0.085, seed: 157 },
    mat: {
      scale: 2.5,
      parallax: 0.015,
      detile: 0.4,
      detail: [9, 0.92, 0.56, 26],
      macro: [0.085, 0.62, 0.24, 0.45],
      macroBig: [2.05, 0.13, 0.028, 0],
      patch: [0.28, 2.0, 0.145, -0.08],
      weather: [0.3, 0.8, 0.7, 0.6],
      wearColor: 0x9a978f,
      dustColor: 0x7f7a68,
      grimeColor: 0x121609,
      roughness: [0.98, -0.01, 0.24],
