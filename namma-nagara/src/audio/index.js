...
import { Ambience, ambientOneShot, ONE_SHOTS, ONE_SHOTS_INDIA } from './ambience.js';
...
export class AudioSystem {
  ...
  async start() {
    ...
    try {
      ...
      // Which ambient one-shot table this level draws from. The Bengaluru
      // street is horns and two-stroke autos, not helicopters and sirens.
      this._oneShotTable =
        this.ctx?.config?.level === 'bengaluru' ? ONE_SHOTS_INDIA : ONE_SHOTS;
      this.ambience.start();
  ...
  _ambientOneShot() {
    ...
    const rng = this.rng;
    const lp = this.field.listenerPos;
    // The Bengaluru street runs a horn-dominated table; see ONE_SHOTS_INDIA.
    const which = rng.pick(this._oneShotTable ?? ONE_SHOTS);
