...
export const SHOTS = {
  ...
  // ---- bengaluru (?level=bengaluru) ----
  // These frame content that only exists on the Indian level. Running them
  // against `market` is harmless — you just get the market street from an
  // unusual angle — so they stay in the one shared table.
  blr_auto: {
    // LEVEL (4, -13) looking at the auto at LEVEL (8.5, -16), through the
    // level transform (yaw 0.5877, tx 0.9, tz 1.34):
    //   x' = x·cos - ... see Assembler.setTransform. Do the arithmetic rather
    //   than eyeballing it — the first version of this shot put the camera
    //   inside a building.
    // Shot from INSIDE the gravel lane (LEVEL 13, -16.5) back along the rank.
    // Framing it from the street instead puts a BESCOM pole across the lens —
    // the poles sit on the kerb line at x = ±5.45, every 12 m.
    pos: [2.6, 1.35, -19.6],
    look: [-0.63, 0.8, -14.69],
    fov: 58,
    time: 16.5,
    doc: 'The auto stand in the east lane — auto-rickshaw geometry and livery.',
  },
  blr_street: {
    pos: [-1.2, 1.7, 6.0],
    look: [-3.6, 1.5, -8.0],
    fov: 75,
    time: 16.5,
    doc: 'Down the carriageway — painted kerbs, speed breakers, drain slabs, chajjas.',
  },
  hud: {
    pos: [12, 1.75, 18],
    look: [-4, 2.2, -6],
