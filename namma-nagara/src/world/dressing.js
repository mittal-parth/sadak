...
/**
 * @param {object} opts
 *   `war` (default true) — place the conflict dressing: barriers, sandbag
 *   emplacements, burnt-out vehicles, shelling rubble, and the drum/crate cover
 *   clusters. A peacetime level turns it off and keeps the rest, which is
 *   everything that is actually about a street: stalls, palms, lamps, overhead
 *   lines, facade hangings, the floor.
 *
 *   The four SET_PIECES-driven passes already place nothing when their arrays
 *   are empty, but `coverClusters` is hardcoded — three oil-drum rings, a
 *   stalled saloon, a tyre pile and a pallet stack — so it has to be gated here.
 *
 *   DEFAULT TRUE ON PURPOSE: the market level calls this with no options and
 *   must draw from the shared RNG stream in exactly the order it always has,
 *   or its baseline shifts. See ARCHITECTURE.md, "The RNG stream is ordered".
 */
export function dressStreet(A, rng, opts = {}) {
  const war = opts.war !== false;
  ...
  A.jitter = jitterRig();
  marketStalls(A, rng);
  if (war) barriers(A, rng);
  if (war) sandbagEmplacements(A, rng);
  if (war) wrecks(A, rng);
  palms(A, rng);
  streetLamps(A, rng);
  overheadLines(A, rng);
  facadeHangings(A, rng);
  if (war) rubblePiles(A, rng);
  tyreStacks(A, rng);
  if (war) coverClusters(A, rng);
  streetFloor(A, rng);
  A.jitter = null;
}
