 ...
 * PER-LEVEL OVERRIDES.
 *
 * A level can remap any palette key onto a different surface without touching
 * the geometry that references it. That matters because `dressing.js`,
 * `kit.js` and `buildings.js` name palette keys in ~200 places and are shared
 * by every level: overriding `plaster_cream` is how the Bengaluru street gets
 * monsoon-stained painted plaster on the same walls the market street renders
 * in desert stucco, with no fork of the builders and no risk to the market
 * level's baseline.
 *
 * `null` (the default, and what `market` uses) means the table below is used
 * verbatim, so the reference level cannot move.
 */
let OVERRIDES = null;
...
/** Resolve a palette key through the active level's overrides. */
export function paletteOf(key) {
  return (OVERRIDES !== null ? OVERRIDES[key] : undefined) ?? PALETTE[key];
}
