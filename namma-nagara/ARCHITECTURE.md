...
| `world` | `src/world/` | levels, level geometry, the modular building kit, props, set dressing, static collision meshes |
...
`world` builds one level, chosen by `config.level` (`?level=<id>`), from the
registry in `src/world/levels/index.js`. A level directory owns its layout,
LEVEL→WORLD transform, spawn points, bounds, point-light slot budget, and — the
important one — **its own build order**.

`src/world/layout.js` is not a map any more; it holds the ACTIVE level's map as
**live `export let` bindings**, swapped by `setLayout()` before anything builds.
`ground.js` and `dressing.js` import those names directly across ~40 call sites
and did not have to change. Exactly one level is loaded at a time.

| id | directory | notes |
|---|---|---|
| `market` | `src/world/levels/market/` | Middle-Eastern market street. **The reference level for the pixel gate** — its build order is load-bearing, see below. |
| `bengaluru` | `src/world/levels/bengaluru/` | Indian street. Under active construction; expected to change every milestone. |

### The RNG stream is ordered, and the gate depends on it

Every builder draws from one shared seeded stream (`ctx.rng.fork()` in
`WorldSystem.init`). Inserting, removing or reordering a single call in a
level's `build()` moves **every subsequent random draw in that level**, and its
baseline shifts. This is the easiest way to break `tools/imagediff.mjs`.

So: to add content, add or edit a *level*, never the shared build order. If you
must register new prototypes globally, register them on a `rng.fork()` so the
draws land in a side stream.
